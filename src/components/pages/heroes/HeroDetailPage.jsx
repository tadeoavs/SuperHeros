...
export default function HeroDetailPage() {
    ...
    useEffect(() => {
        const fetchHeroData = async () => {
            try {
                const response = await fetch(`https://www.superheroapi.com/api.php/7b85261e5c06bfbc1cb085ff657641dd/search/${heroName}`);
                if (!response.ok) {
    ...
    if (error) {
        return <Typography variant="h4">Error al cargar los datos del héroe</Typography>;
    }

    if (!hero) {
        return <Typography variant="h4">Héroe no encontrado</Typography>;
    }
